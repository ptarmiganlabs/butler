...
# Reset any leftover keychain state from previous runs on this machine
...
cd src
zip -u -r "../${DIST_FILE_NAME}-${RELEASE_VERSION}-macos-arm64.zip" "./config/email_templates" "./config/slack_templates" "./config/teams_templates" "./config/production_template.yaml" "./config/schedule_template.yaml" "./config/log_appender_xml" -x "*.DS_Store"
cd ..
