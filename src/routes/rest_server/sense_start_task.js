...
async function handlerPutStartTask(request, reply) {
    try {
        ...
        // Fastify schema validation ensures taskId parameter is present and non-empty
        ...
        if (request.params.taskId === '-') {
            ...
            if (
                ...
                if (taskExists.exists) {
                    ...
                    globals.logger.silly(`STARTTASK: Added task to invalid taskId array, now ${tasksInvalid.length} entries in that array`);
                }
            } else {
        ...
        if (request.body && Object.keys(request.body).length > 0) {
            ...
            for (const item of request.body) {
                if (item.type === 'keyvaluestore') {
                    ...
                    if (
                        taskFilterEnabled === false ||
                        (taskFilterEnabled === true && isTaskCPAllowed(item.payload.customPropertyName, item.payload.customPropertyValue))
                    ) {
