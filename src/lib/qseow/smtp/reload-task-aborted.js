...
import { isSmtpConfigOk, isEmailReloadAbortedNotificationConfigOk, rateLimiterMemoryAbortedReloads } from './config.js';
    ...
    for (const recipientEmailAddress of mainSendListUnique) {
        rateLimiterMemoryAbortedReloads
            ...
            .then(async (rateLimiterRes) => {
                try {
                    globals.logger.debug(
                        `[QSEOW] EMAIL RELOAD TASK ABORTED ALERT: Rate limit check passed for ${recipientEmailAddress}, task "${reloadParams.taskName}". Remaining points: ${rateLimiterRes.remainingPoints}`,
                    );
            ...
            .catch((err) => {
                globals.logger.warn(
                    `[QSEOW] EMAIL RELOAD TASK ABORTED ALERT: Rate limit exceeded. Not sending reload notification email for task "${reloadParams.taskName}" to "${recipientEmailAddress}". Wait before sending another email to this recipient for this task.`,
                );
                globals.logger.debug(`[QSEOW] EMAIL RELOAD TASK ABORTED ALERT: Rate limiter details: ${JSON.stringify(err, null, 2)}`);
            });
    }
}
